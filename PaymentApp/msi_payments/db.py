...
GET_SQL = """
SELECT id, amount, currency, payer, payee, description, external_id, is_return, status, error
FROM msipay
WHERE id = $1
"""
...
INSERT_SQL = """
INSERT INTO
    msipay(id, amount, currency, payer, payee, description, external_id, is_return, status, error)
...
class Database:
    ...
    async def get_payment(self, payment_id: uuid.UUID) -> typing.Optional[Payment]:
        ...
        if db_payment:
            return Payment.parse_obj(db_payment)
        else:
            return None

    async def insert_payment(self, payment: Payment):
        await self._insert_stmt.fetch(
            payment.id,
            payment.amount,
            payment.currency,
            payment.payer,
    ...
    async def update_payment_status(self, payment: Payment):
        await self._update_stmt.fetch(payment.id, payment.status.value, payment.error)
